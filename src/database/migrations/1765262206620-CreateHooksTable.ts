...
export class CreateHooksTable1765262206620 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`hooks\` (\`id\` int NOT NULL AUTO_INCREMENT, \`userId\` int NULL, \`text\` text NOT NULL, \`is_active\` tinyint NULL DEFAULT 1, \`createdAt\` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
