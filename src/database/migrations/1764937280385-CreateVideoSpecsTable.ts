...
export class CreateVideoSpecsTable1764937280385 implements MigrationInterface {
  name = 'CreateVideoSpecsTable1764937280385';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`video_specs\` (\`id\` int NOT NULL AUTO_INCREMENT, \`length\` int NOT NULL, \`goal\` varchar(255) NOT NULL, \`createdAt\` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
