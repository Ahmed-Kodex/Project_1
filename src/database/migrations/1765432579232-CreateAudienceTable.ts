...
export class CreateAudienceTable1765432579232 implements MigrationInterface {
  name = 'CreateAudienceTable1765432579232';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`audience\` (\`id\` int NOT NULL AUTO_INCREMENT, \`userId\` int NULL, \`targetAudience\` varchar(255) NOT NULL, \`ageRange\` varchar(255) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`audience\` ADD CONSTRAINT \`FK_4960a4ab78d89b185e3d703b789\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
  }
